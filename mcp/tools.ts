...
export async function createItineraryTool(params: {
  ...
  interests?: string[];
  notes?: string;
}): Promise<{ status: string; itinerary_id: string; message: string; itinerary?: Itinerary; source: 'plantrip-api' | 'sample' }> {
  ...
  if (planTripHeaders) {
    try {
      ...
      console.warn('PlanTrip API unavailable, generating sample itinerary:', err?.message);
    }
  }
  ...
  return {
    ...
    message: `Created a ${duration}-day sample itinerary for ${destination} (illustrative, generated by this app).`,
    itinerary,
    source: 'sample'
  };
}
...
export async function getItineraryStatusTool(params: {
  itinerary_id: string;
}): Promise<{ itinerary_id: string; status: string; progress: number; message: string; found?: boolean }> {
  const job = itineraryJobs.get(params.itinerary_id);
  if (!job) {
    if (inMemoryTrips.has(params.itinerary_id)) {
      return {
    ...
    return {
      ...
      progress: 0,
      found: false,
      message: `No itinerary job with id ${params.itinerary_id} on this server instance.`
    };
  }
...
export async function getItineraryTool(params: {
  ...
  return {
    itinerary: null,
    found: false,
    message: `Itinerary ${params.itinerary_id} was not found on this server instance.`
  };
}
...
export async function modifyItineraryTool(params: {
  ...
  itinerary?: Itinerary;
}): Promise<{ success: boolean; itinerary: Itinerary | null; message: string; found?: boolean }> {
  // Serverless instances do not share memory, so the caller may send the itinerary it holds.
  const itinerary =
    inMemoryTrips.get(params.itinerary_id) ??
    (params.itinerary && params.itinerary.id === params.itinerary_id && Array.isArray(params.itinerary.days)
      ? params.itinerary
      : undefined);
...
export async function deleteTripTool(params: {
  trip_id: string;
}): Promise<{ success: boolean; message: string; found?: boolean }> {
...
export async function submitTourInquiryTool(params: {
  ...
  return {
    ...
    message: `Demo inquiry recorded for "${title}" with reference ${code}. This demo does not book tours or send email; contact a tour operator to reserve.`,
